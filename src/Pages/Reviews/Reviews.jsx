...
class Reviews extends ReviewsController {
  ...
  render() {
    // Alias del estado
    const { reviews, viewAnswers, reviewSelected, filters } = this.state;
    ...
    const Columns = [
      ...
      },
      {
        ...
        render: (rowData) => (
          <Tooltip
            arrow
            title={
              <SugestionText variant="h6">{rowData.suggestions}</SugestionText>
            }
          >
            <span>{rowData.suggestions.substr(0, 20)}</span>
          </Tooltip>
        ),
      },
      {
      ...
      },
      {
        ...
        render: (rowData) => (
          <span>{QUALIFICATIONS[rowData.average.qualification]}</span>
        ),
      },
    ];
    ...
    if (user.store === STORES.ALL) {
      ...
      const Userstores = user.store.split(",");
          ...
          <Modal>
            ...
            <ModalContent>
                ...
                <MaterialTable
                  columns={[
                    ...
                    },
                    {
                      ...
                      field: "qualification",
                      render: (rowData) => (
                        <span>{QUALIFICATIONS[rowData.qualification]}</span>
                      ),
                    },
                  ]}
                  data={reviewSelected.answers}
                  title={`${
                    reviewSelected.average.percent
                  }% de las respuestas fueron ${
                    QUALIFICATIONS[reviewSelected.average.qualification]
                  }`}
                  options={{
                    search: false,
                    filtering: false,
              ...
              <Footer>
                <CloseButton
                  variant="contained"
                  onClick={() => this.setState({ viewAnswers: false })}
                >
                  Cerrar
                </CloseButton>
              </Footer>
